...
def print_ghost(ghost_dict):
    ...
    output = f"""
    This is the ghost of {ghost_dict["first_name"]} '{ghost_dict["aliases"]}' {ghost_dict["family_name"]}.
    There is/are (a/an) {ghost_dict["ghostly_effect"]} when this {ghost_dict["ghost_trait"]} spirit appears!
    """
    print(output)
    return output
